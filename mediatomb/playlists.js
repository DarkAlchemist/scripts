...
//var playlistChain = createContainerChain(new Array('Playlists', 'All Playlists', abcbox(playlist_title, '', ''), playlist_title));
var playlistChain = createContainerChain(new Array('Playlists', abcbox(playlist_title, '', ''), playlist_title));
var internetRadioChain = createContainerChain(new Array('Internet Radio', playlist_title));
...
else if (type == 'm3u')
{
    ...
    do
    {
        ...
        else if (! line.match(/^(#|\s*$)/))
        {
			...
			// Leaving this out as I don't give a crap about the directory location
            //if (playlistDirChain)
            ...
            if (lastId != id)
            {
                if (file){
					if ( file.indexOf("http:") !== -1 ){
						addPlaylistItem(file, title, internetRadioChain, lastId);
					}
					else{
